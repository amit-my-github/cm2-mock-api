...
namespace Asp.Net.Core.WebApi.CrudOperations.Services
{
    public static class FacetsServiceProvider
    {
        ...
        public static FacetsResponse GetFacetCategoryMapping(FacetsQueryParameters parameters)
        {
            var res = new FacetsResponse();
            var connectionString = GetConnection();
            var facets = GetFacetCategoryMapping(connectionString, parameters);
            if (facets != null && facets.Tables.Count > 0 && facets.Tables[0].Rows.Count > 0)
            {
                var totalItems = 0;
                foreach (var facet in facets.Tables[0].AsEnumerable())
        ...
        private static DataSet GetFacetCategoryMapping(string connectionString, FacetsQueryParameters parameters)
        {
