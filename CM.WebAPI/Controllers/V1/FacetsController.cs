...
namespace CM.WebAPI.Controllers.V1
{
    ...
    [ApiController]
    [ApiVersion("1.0")]
    [Route("api/v{version:apiVersion}/facets-categories")]
    public class FacetsController : ControllerBase
    {
        ...
        public IActionResult Create([FromBody] CategoriesItem accessCode)
        {
            ...
            if (res > 0)
            {
                msg = JsonSerializer.Serialize(new { status = "SUCCESS", Message = "Category has been successfully added!" });
            }
            else
            {
        ...
        public IActionResult Update(int id, [FromBody] CategoriesItem accessCode)
        {
            ...
            if (res)
            {
                var msg = JsonSerializer.Serialize(new { status = "SUCCESS", Message = "Category has been updated successfully!" });
                return Ok(msg);
            }
            return NoContent();
        ...
        public IActionResult Delete(int id)
        {
            ...
            if (res)
            {
                var msg = JsonSerializer.Serialize(new { status = "SUCCESS", Message = "Category has been deleted successfully!" });
                return Ok(msg);
            }
            return NoContent();
        }
        ...
        //[HttpGet]
        //public ActionResult<CategoriesMappingResponse> GetFacetCategoryMapping(string categoryName, int categoryId, FacetsQueryParameters parameters)
        //{
        //    var mapping = Services.FacetsServiceProvider.GetFacetCategoryMapping(categoryName, categoryId, parameters);
        //    if (mapping != null)
        //    {
        //        return mapping;
        //    }
