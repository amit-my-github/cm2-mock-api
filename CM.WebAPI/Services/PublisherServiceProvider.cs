...
namespace CM.WebAPI.Services
{
    public static class PublisherServiceProvider
    {
        ...
        public static int Create(AddPublisherRequest publisher)
        {
            var connectionString = GetConnection();
            return CreatePublisher(connectionString, publisher);
        }

        public static bool Update(int Id, AddPublisherRequest publisher)
        {
            var connectionString = GetConnection();
        ...
        {
            var connectionString = GetConnection();
            return RemovePublisher(connectionString, id) > 0;
        }
        ...
        private static int CreatePublisher(string connectionString, AddPublisherRequest publisher)
        {
            if (publisher != null)
            {
                using (var objDd = new MsSqlClient(connectionString))
        ...
        private static bool IsValid(string connectionString, int publisherId)
        {

            using (var objDd = new MsSqlClient(connectionString))
            {
                objDd.AddParameter("@PublisherId", publisherId);
                var obj = objDd.ExecuteDataSet("uspIsValidPublisherId", CommandType.StoredProcedure);
