...
namespace CM.WebAPI.Services
{
    public static class PublicationServiceProvider
    {
        ...
        public static PublicationTitleMappingAndRelationResponse? GetMappingAndRelations(int id)
        {

            var connectionString = GetConnection();
            var publication = GetPublicationTitleMappingAndRelationsById(connectionString, id);
            if (publication != null && publication.Tables.Count > 0 )
            {
                var pub = new PublicationTitleMappingAndRelationResponse();
                pub.PublicationTitleRelations = new PublicationTitleRelations();
                if (publication != null && publication.Tables.Count > 0 && publication.Tables[0].Rows.Count > 0)
                {
                    pub.Title = new Title
                    {
                        PublicationTitleId = publication.Tables[0].Rows[0].Field<int>("ID"),
                        DisplayCode = publication.Tables[0].Rows[0].Field<string?>("Displaycode"),
                        Type = publication.Tables[0].Rows[0].Field<string?>("Type"),
                        DateCreated = publication.Tables[0].Rows[0].Field<DateTime>("dateCreated").ToString("yyyy-MM-ddTHH:mm:ss"),
                        LastModified = publication.Tables[0].Rows[0].Field<DateTime>("lastmodified").ToString("yyyy-MM-ddTHH:mm:ss"),
                        AccessCode = publication.Tables[0].Rows[0].Field<string?>("AccessCode"),
                        Name = publication.Tables[0].Rows[0].Field<string?>("Name"),
                        Notes = publication.Tables[0].Rows[0].Field<string?>("Notes"),
                        ExternalCode = new List<string>(),
                        ThesaurusSynonyms = new List<string>()
                    };
                    var exts = publication.Tables[0].Rows[0].Field<string?>("ExternalCode").Split(',');
                    var syns = publication.Tables[0].Rows[0].Field<string?>("Synonym").Split(',');
                    foreach (var item in exts)
                    {
                        pub.Title.ExternalCode.Add(item);
                    }
                    foreach (var item in syns)
                    {
                        pub.Title.ThesaurusSynonyms.Add(item);
                    }
                }
                //if (publication != null && publication.Tables.Count > 0 && publication.Tables[2].Rows.Count > 0 || )
                ...
                if (publication != null && publication.Tables.Count > 0 && publication.Tables[2].Rows.Count > 0)
                {
                    pub.PublicationTitleRelations.PartOf = new RelationItem()
                    {

                        PublicationTitlesId = publication.Tables[0].Rows[0].Field<int>("ID"),
                        DisplayCode = publication.Tables[0].Rows[0].Field<string?>("Displaycode"),
                        Type = publication.Tables[0].Rows[0].Field<string?>("Type"),
                        AccessCode = publication.Tables[0].Rows[0].Field<string?>("AccessCode"),
                ...
                // TitleOf
                if (publication != null && publication.Tables.Count > 0 && publication.Tables[3].Rows.Count > 0)
                {
                    pub.PublicationTitleRelations.TitleOf = new List<RelationItem>();
                    foreach (var item in publication.Tables[3].AsEnumerable())
                    {
                        pub.PublicationTitleRelations.TitleOf.Add(new RelationItem
                        {
                            PublicationTitlesId = item.Field<int>("ID"),
                            DisplayCode = item.Field<string?>("Displaycode"),
                            Type = item.Field<string?>("Type"),
                            AccessCode = item.Field<string?>("AccessCode"),
                        });
                    }
                }
                //--Given Access To 4--
                if (publication != null && publication.Tables.Count > 0 && publication.Tables[4].Rows.Count > 0)
                {
                    pub.PublicationTitleRelations.GivesAccessTo = new List<RelationItem>();
                    foreach (var item in publication.Tables[3].AsEnumerable())
                    {
                        pub.PublicationTitleRelations.GivesAccessTo.Add(new RelationItem
                        {
                            PublicationTitlesId = item.Field<int>("ID"),
                            DisplayCode = item.Field<string?>("Displaycode"),
                            Type = item.Field<string?>("Type"),
                ...
                //-- Accessed Via 5--
                if (publication != null && publication.Tables.Count > 0 && publication.Tables[5].Rows.Count > 0)
                {
                    pub.PublicationTitleRelations.AccessedVia = new List<RelationItem>();
                    foreach (var item in publication.Tables[3].AsEnumerable())
                    {
                        pub.PublicationTitleRelations.AccessedVia.Add(new RelationItem
                        {
                            PublicationTitlesId = item.Field<int>("ID"),
                            DisplayCode = item.Field<string?>("Displaycode"),
                            Type = item.Field<string?>("Type"),
        ...
        private static DataSet GetPublicationTitleMappingAndRelationsById(string connectionString, int publicationTitleId)
        {

            using (var objDd = new MsSqlClient(connectionString))
            {
                objDd.AddParameter("@PublicationTitleId", publicationTitleId);
                return objDd.ExecuteDataSet("uspGetPublicationTitleMappingAndRelationDetailsById", CommandType.StoredProcedure);
            }
        }
